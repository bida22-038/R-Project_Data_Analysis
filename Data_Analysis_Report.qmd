...
- January, February & March are assigned to the first quarter.
- April, May & June are assigned to the second quarter.
- July, August, September are assigned to the third quarter.
- October, November & December are assigned to the fourth quarter.
...
- The lower bound is calculated by subtracting the product of the IQR and the threshold (1.5), from the first quantile.
- The upper bound is calculated by subtracting the product of the IQR and the threshold from the third quantile.

The threshold used in bound calculation (1.5) is a universal threshold applied in outlier detection using IQR. This balances discovery of extreme points and identification of normal data points.
The code filters any data point that is outside of the range (i.e., the data point’s value is less than the lower bound or greater than the upper bound.)
Below are boxplots of Closing Price showing before and after outlier handling:
...
The blue line indicates the forecast line, which is the values predicted using the chosen ARIMA model from **auto.arima()**.

## 5.5 ARIMA Forecast Accuracy Evaluation

```{r}
accuracy_metrics <- accuracy(forecast_30_arima, testing_data$Close)
accuracy_df <- as.data.frame(accuracy_metrics)

view(accuracy_df)

```
accuracy() is then used to extract accuracy metrics to determine how well the model correctly predicted the values. **accuracy()** provides the following metrics:
...
Using the metrics below, and the information above, they suggest that the ARIMA model performed poorly with test data, compared with the training data's metrics. 
...
-   Wickham, H. and Grolemund, G., 2017. *R for Data Science*. 1st ed. O'Reilly Media. Available at: <https://r4ds.had.co.nz/> \[Accessed 15 October 2024\].

-   Williams, G.J, 2018. *The Essentials of Data Science: Knowledge Discovery Using R*. 1st ed. CRC Press. Available at: <https://doi.org/10.1201/9781315151458> \[Accessed 20 October 2024\].

-   Hadi Rezaei, Hamidreza Faaljou, Gholamreza Mansourfar, *Stock price prediction using deep learning and frequency decomposition*, *Expert Systems with Applications*, Volume 169, 2021. Available at: <https://doi.org/10.1016/j.eswa.2020.114332>.

-   Huang, Shian-Chang. (2017). *A Big Data Analysis System for Financial Trading*. Global Business and Finance Review, 22, 32-44. Available at: <https://doi.org/10.17549/gbfr.2017.22.3.32>.

-   Grolemund, G., & Wickham, H. (2017). *R for Data Science: Import, Tidy, Transform, Visualize, and Model Data*. O'Reilly Media.

-   Waring, E., et al. (2020). *skimr: Compact and Flexible Summaries of Data*. R package version 2.1.3.
...
-   Tsay, R.S., 2010. *Analysis of Financial Time Series*. 3rd ed. New Jersey: Wiley.
...
-   Bouri, E., Molnár, P., Azzi, G., Roubaud, D., and Hagfors, L.I., 2020. *On the Hedge and Safe Haven Properties of Bitcoin: Is It Really More Than a Diversifier?* *Finance Research Letters*, 20, pp.192-198.

-   Corbet, S., Larkin, C., and Lucey, B., 2020. *The Contagion Effects of the COVID-19 Pandemic: Evidence from Gold and Cryptocurrencies*. *Finance Research Letters*, 35, p.101554.
...
-   Hyndman, R.J. and Athanasopoulos, G. (2018). *Forecasting: Principles and Practice*, 2nd ed., OTexts, Melbourne, Australia. Section 3.4 "Evaluating Forecast Accuracy."

-   Sahay, A. M. (2021). *Business Analytics: A Data-Driven Decision-Making Approach for Business, Volume II*. New York: Springer.

-   Crypto Engineer (2024). *Cryptocurrency Investing: The Fundamental Analysis Guide*. Cambridge: PublishDrive.

-   Huang, L. (2023). *Data and Intuition: Good Decisions Need Both*. Harvard Business Publishing.
